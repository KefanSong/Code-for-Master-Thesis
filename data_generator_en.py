...
class DataGenerator:
    ...
    def run_traj(self, env, policy, value_net, cvalue_net, running_stat,
                 ...
                 dtype, device, constraint, task_id=-1):
    ...
    def get_advantage(self, value_net, gamma, gae_lam, dtype, device, mode='reward'):
        if mode == 'reward':
            ...
            for t in reversed(range(self.eps_len)):
                ...
                # print(self.task_id)
                current_val, next_val = torch_to_numpy(value_net(obs_tensor,self.task_id), value_net(next_obs_tensor,self.task_id))
            ...
            vtarg_eps = torch_to_numpy(value_net(obs_eps_tensor,self.task_id)) + adv_eps
