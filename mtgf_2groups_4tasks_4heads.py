...
import wandb
wandb.login()
wandb.init(project="mtgf-1000-2g4t-4heads")
...
class FOCOPS:
    ...
    # TO-DO: Add rollout3 and rollout4 as input, other_group_all_tasks_returns
    def update_params(self, rollouts, dtype, device,return_diff, all_return_diff, group_id, task_id):
        ...
        self.nu[z][3] -= self.nu_lr * (self.epsilon + return_diff[1])
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b, adv_b2, adv_b3, adv_b4, cvtarg_b, cadv_b,
                ...
                # weight decay
                # weight decay only for a particular task id.
                for param in self.value_net.value_policies[task_id].parameters():
                ...
                # calculate adv_b2 and adv_coefficient2, and add it to the policy update. 
                if task_id == 1:
                    adv_coefficient2 = 1.0
                else:
                    adv_coefficient2 = 0
                # for z in range(self.num_subgroups - 1):
                z = group_id
                ...
                # TO-DO: calculate adv_coefficient3, 4, and add it to the policy update.

                if task_id == 2:
                    adv_coefficient3 = 1.0
                else:
                    adv_coefficient3 = 0
                ...
                # elif task_id == 1:
                #     policy_update_term = adv_b2 * adv_coefficient2
                # elif task_id ==2:
                #     policy_update_term = adv_b3 * adv_coefficient3
                # elif task_id ==3:
                #     policy_update_term = adv_b4 * adv_coefficient4

                policy_update_term = adv_b * adv_coefficient + adv_b2 * adv_coefficient2 + adv_b3 * adv_coefficient3 + adv_b4 * adv_coefficient4
...
envnames = ['HalfCheetah-v3',
    'BigFootHalfCheetah', 'HugeGravityHalfCheetah','TenFricHalfCheetah']
...
def make_envs(args):
    ...
    # # TO-DO: copy two more for two more groups
    # tasks = []
    # for t in range(4):
...
def train(args):
    ...
    for z in range(num_subgroups):
        ...
        env = envs[z][0]
        envname = envnames[0]
        # change it to multi task Agent, check task_id
        agent = MultiTaskAgent(env,envname,args,device=device,num_tasks=num_tasks)
        ...
        for z0 in range(num_subgroups):
            ...
            for _ in range(args.rounds_of_update):
                for t in range(num_tasks):
                    ...
                    wandb.log({"fair_gap Task"+str(t): all_return_diff[t]})
