...
import wandb
wandb.login()
wandb.init(project="mtgf-1000-4g2t")
    ...
    class HalfCheetahRewardWrapper(gym.Wrapper):
        ...
        def step(self, action):
            ...
            if self.task_id == 0:
                ...
                vel_reward = -1*np.abs(info['x_velocity'] - 0.1*target_velocity) + 0.1*target_velocity
                ...
                target_velocity = 20
                vel_reward = -1*np.abs(info['x_velocity'] - 0.1*target_velocity)/20*5 + 0.1*5
                ctrl_cost = info['reward_ctrl']
                reward = vel_reward + ctrl_cost
