...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device, group):
        ...
        dataset = torch.utils.data.TensorDataset(obs, act, vtarg, adv0, adv1, cvtarg, cadv,
                                                 old_logprob, old_mean, old_std)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b0, adv_b1, cvtarg_b, cadv_b,
                    old_logprob_b, old_mean_b, old_std_b) in enumerate(loader):
    ...
    # traditinal update_parameter
    def update_params_traditional(self, rollout, dtype, device, group):
        ...
        # self.logger.update('AvgR', np.mean(self.score_queue))
        # get rid of the lowest 5 values when calculating the score
        self.logger.update('AvgR', np.mean(np.sort(self.score_queue)))
        self.logger.update('MinC', np.min(self.cscore_queue))
        self.logger.update('MaxC', np.max(self.cscore_queue))
        self.logger.update('AvgC', np.mean(self.cscore_queue))
        # self.logger.update('nu', self.nu)
    ...
    def update_params_task1(self, rollout, dtype, device, group):
        ...
        # Get log likelihood, mean, and std of current policy
        old_logprob, old_mean, old_std = self.policy.logprob(obs, act, task=1)
        old_logprob, old_mean, old_std = to_dytype_device(dtype, device, old_logprob, old_mean, old_std)
        old_logprob, old_mean, old_std = graph_detach(old_logprob, old_mean, old_std)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b0, adv_b1, cvtarg_b, cadv_b,
                ...
                # Update policy
                logprob, mean, std = self.policy.logprob(obs_b, act_b, task=1)
                kl_new_old = gaussian_kl(mean, std, old_mean_b, old_std_b)
                ratio = torch.exp(logprob - old_logprob_b)
                # self.pi_loss = (kl_new_old - (1 / self.lam) * ratio * (adv_b - self.nu * cadv_b[:,:,:])) \
                #           * (kl_new_old.detach() <= self.eta).type(dtype)
                ...
                self.pi_optimizer.zero_grad()
                self.pi_loss.backward()
                # To check whether we need to exclude task2's parameters
                torch.nn.utils.clip_grad_norm_(self.policy.parameters(), 40)
                self.pi_optimizer.step()


            # Early stopping
            logprob, mean, std = self.policy.logprob(obs, act, task=1)
            kl_val = gaussian_kl(mean, std, old_mean, old_std).mean().item()
            if kl_val > self.delta:
                println('Break at epoch {} because KL value {:.4f} larger than {:.4f}'.format(epoch + 1, kl_val, self.delta))
                break
...
def train(args, env_list, envname, load_model, cost_lim, group, score_queue, cscore_queue):
    ...
    for iter in range(args.max_iter_num):
        ...
        rollout = data_generator.run_traj(env_list[0], agent.policy, agent.value_net_list, agent.cvalue_net,
        ...
        # use traditional rollout; change update_parameter_task1 to fit traditional rollout
        
        # agent.update_params_task1(rollout, dtype, device, group)
        agent.update_params_traditional(rollout, dtype, device, group)
