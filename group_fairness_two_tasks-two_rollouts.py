    ...
    def update_params(self, rollout,rollout2, dtype, device):
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b0,obs_b1, act_b, vtarg_b, adv_b0, adv_b1, cvtarg_b, cadv_b,
                ...
                self.pi_loss = (kl_new_old - (1 / self.lam) * ratio * (adv_b_list[1])) \
                          * (kl_new_old.detach() <= self.eta).type(dtype)
...
def train(args, env_list, envname, load_model, cost_lim, group, score_queue, cscore_queue):
    ...
    for iter in range(args.max_iter_num):
        ...
        # calling the second DataGenerator2
        data_generator = DataGenerator2(obs_dim, act_dim, args.batch_size, args.max_eps_len)
        rollout2 = data_generator.run_traj(env_list, agent.policy, agent.value_net_list, agent.cvalue_net,
                                          running_stats, agent.score_queue, agent.cscore_queue,
                                          args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
                                          dtype, device, args.constraint)
