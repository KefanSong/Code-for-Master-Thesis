...
import time
from data_generator_two_tasks_debug_rollout2 import DataGenerator as DataGenerator2
from data_generator_two_tasks_debug_rollout1 import DataGenerator
...
class FOCOPS:
    ...
    def update_params(self, rollout,rollout2, dtype, device):
        ...
        adv0 = rollout['advantages'][0]
        adv1 = rollout2['advantages'][0]
        ...
        old_logprob1, old_mean1, old_std1 = self.policy.logprob(obs1, act)

        old_lobprob = torch.mean(torch.stack([old_logprob, old_logprob1]), dim=0)
        old_mean = torch.mean(torch.stack([old_mean, old_mean1]), dim=0)
        old_std = torch.mean(torch.stack([old_std, old_std1]), dim=0)
        ...
        dataset = torch.utils.data.TensorDataset(obs, obs1, act, vtarg, adv0, adv1, cvtarg, cadv,
                                                 old_logprob, old_mean, old_std)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, obs_b1, act_b, vtarg_b, adv_b0, adv_b1, cvtarg_b, cadv_b,
                    old_logprob_b, old_mean_b, old_std_b) in enumerate(loader):
                ...
                logprob1, mean1, std1 = self.policy.logprob(obs_b1, act_b)

                
                logprob = torch.mean(torch.stack([logprob, logprob1]), dim=0)
                mean = torch.mean(torch.stack([mean, mean1]), dim=0)
                std = torch.mean(torch.stack([std, std1]), dim=0)
            ...
            logprob1, mean1, std1 = self.policy.logprob(obs1, act)
...
def train(args, env_list, envname, load_model, cost_lim, group, score_queue, cscore_queue):
    ...
    for iter in range(args.max_iter_num):
        ...
        data_generator2 = DataGenerator2(obs_dim, act_dim, args.batch_size, args.max_eps_len)
        rollout2 = data_generator2.run_traj(env_list, agent.policy, agent.value_net_list, agent.cvalue_net,
                                          running_stats, agent.score_queue, agent.cscore_queue,
                                          args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
                                          dtype, device, args.constraint)

        # rollout0 = data_generator.run_traj(env_list[0], agent.policy, agent.value_net_list[0], agent.cvalue_net,
        #                                   running_stats, agent.score_queue, agent.cscore_queue,
        #                                   args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
        #                                   dtype, device, args.constraint)
        # rollout1 = data_generator.run_traj(env_list[1], agent.policy, agent.value_net_list[1], agent.cvalue_net,
        #                                   running_stats, agent.score_queue, agent.cscore_queue,
        #                                   args.gamma, args.c_gamma, args.gae_lam, args.c_gae_lam,
        #                                   dtype, device, args.constraint)
