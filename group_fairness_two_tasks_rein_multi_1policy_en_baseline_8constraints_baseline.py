...
import wandb
wandb.login()
wandb.init(project="tmlr-500-2g4t")
...
class FOCOPS:
    ...
    def __init__(self,
        ...
        self.nu = np.ones((self.num_subgroups, 8)) * initial_nu_value
        
    # add the second rollout as input; the other group return on the second task as input
    def update_params(self, rollout,rollout2, rollout3, rollout4, dtype, device,return_diff, other_group_return,other_group_other_task_return2,other_group_other_task_return3,other_group_other_task_return4,  group_id, task_id):
        ...
        self.nu[z][2] -= self.nu_lr * (self.epsilon + other_group_other_task_return2 - avg_cost2)
        self.nu[z][2] = max(min(self.nu[z][2], self.nu_max), 0)
        self.nu[z][3] -= self.nu_lr * (self.epsilon - other_group_other_task_return2 + avg_cost2)
        self.nu[z][3] = max(min(self.nu[z][3], self.nu_max), 0)

        self.nu[z][4] -= self.nu_lr * (self.epsilon + other_group_other_task_return3 - avg_cost3)
        self.nu[z][4] = max(min(self.nu[z][4], self.nu_max), 0)
        self.nu[z][5] -= self.nu_lr * (self.epsilon - other_group_other_task_return3 + avg_cost3)
        self.nu[z][5] = max(min(self.nu[z][5], self.nu_max), 0)

        # update nu for second task constraints
    
        self.nu[z][6] -= self.nu_lr * (self.epsilon + other_group_other_task_return4 - avg_cost4)
        self.nu[z][6] = max(min(self.nu[z][6], self.nu_max), 0)
        self.nu[z][7] -= self.nu_lr * (self.epsilon - other_group_other_task_return4 + avg_cost4)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b, adv_b2, adv_b3, adv_b4, cvtarg_b, cadv_b,
                ...
                adv_coefficient3 = 0
                # for z in range(self.num_subgroups - 1):
                adv_coefficient3 += (-self.nu[z][4] + self.nu[z][5])
    ...
    class HalfCheetahRewardWrapper(gym.Wrapper):
        ...
        def step(self, action):
            ...
            if self.task_id == 0:
                ...
                vel_reward = -1*np.abs(info['x_velocity'] - target_velocity)/9*3 + 0.3
                ctrl_cost = info['reward_ctrl']
                reward = vel_reward + ctrl_cost
            else:
                obs = np.append(obs, [12, 13, 14, 15])
            ...
            for _ in range(args.rounds_of_update):
                for t in range(num_tasks):
                    ...
                    for z1 in range(num_subgroups):
                        ...
                        other_group_other_task_return2 = rollouts[z1][task_indices[0]]['avg_return']
                        other_group_other_task_return3 = rollouts[z1][task_indices[1]]['avg_return']
                        other_group_other_task_return4 = rollouts[z1][task_indices[2]]['avg_return']
                    ...
                    other_group_return = fcpo[z0][t].update_params(rollouts[z0][t],rollouts[z0][task_indices[0]], rollouts[z0][task_indices[1]], rollouts[z0][task_indices[2]], dtype, device,return_diff,other_group_return,other_group_other_task_return2,other_group_other_task_return3, other_group_other_task_return4, z0, t)
