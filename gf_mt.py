...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device):
        ...
        cadv = torch.Tensor(np.array(rollout['c_advantages'])).to(dtype).to(device).detach()
        ...
        avg_cost_vector = rollout['avg_cost']


        # Update nu
        # self.nu += self.nu_lr * (avg_cost_vector - self.cost_lim)
        self.nu[0] += self.nu_lr * (avg_cost_vector[0] - self.cost_lim[0])
        self.nu[1] += self.nu_lr * (avg_cost_vector[1] - self.cost_lim[1])
        ...
        if self.nu[1] < 0:
            self.nu[1] = 0
        elif self.nu[1] > self.nu_max:
            self.nu[1] = self.nu_max
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b, cvtarg_b, cadv_b,
                ...
                self.pi_loss = (kl_new_old - (1 / self.lam) * ratio * (adv_b - self.nu[0] * cadv_b[:, 0, :] - 0*self.nu[1]*cadv_b[:, 1, :])) \
                          * (kl_new_old.detach() <= self.eta).type(dtype)
        ...
        self.logger.update('nu', self.nu[0])
...
def train(args):
    ...
    for z in range(args.num_groups):
        ...
        # Set initial cost constraint as infinity to sample returns. 
        cost_lim = np.inf
        ...
        # initialize a new logger for each group
    ...
    for k in range(10):
        for z in range(args.num_groups):
            ...
            for z1 in range(args.num_groups):
                ...
                data_generator = DataGenerator(obs_dim, act_dim, args.batch_size*1, args.max_eps_len)
            ...
            for iter in range(args.max_iter_num):
                ...
                #     print(key, np.shape(rollouts[key]))
                # break
...
if __name__ == '__main__':
    ...
    parser.add_argument('--group-fairness-threshold', default=100,
    ...
    parser.add_argument('--nu', type=list, default=[0]*2,
                        help='Cost coefficient (default: 0)')
