    ...
    def update_params(self, rollout, dtype, device):
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b, cvtarg_b, cadv_b,
                ...
                self.pi_loss = (kl_new_old - (1 / self.lam) * ratio * (adv_b - self.nu * cadv_b[:, 0, :])) \
...
def train(args):
    ...
    for z in range(args.num_groups):
        ...
        cost_lim = np.inf
        cost_lim = np.array((np.inf, -np.inf))
    ...
    for k in range(10):
        for z in range(args.num_groups):
            ...
            # test if agent update only update each group's own parameters. 
