...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device, group, epsilon):
        ...
        avg_cost_stack = np.array((avg_cost[0], -avg_cost[0]))
        

        # self.nu += self.nu_lr * (avg_cost_stack - self.cost_lim[:2])


        # self.nu = np.clip(self.nu, a_min=0, a_max= self.nu_max)


        return_diff_stack = np.array((-return_diff, return_diff))

        self.nu -= self.nu_lr * (epsilon - return_diff_stack)
