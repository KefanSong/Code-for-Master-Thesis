...
import time
from data_generator_en_1head import DataGenerator
...
import wandb
wandb.login()
wandb.init(project="mtgf-2g_4t_BigFoot_1head")
...
class FOCOPS:
    ...
    def __init__(self,
        ...
        self.nu = np.ones((self.num_subgroups - 1, 8)) * initial_nu_value
        

    def update_params(self, rollouts, dtype, device,return_diffs, other_group_return, group_id, task_id):
        rollout = rollouts[task_id]
        return_diff = return_diffs[task_id]

        idx_list = [idx for idx in range(4) if idx != task_id]

        adv2 = torch.Tensor(rollouts[idx_list[0]]['actions']).to(dtype).to(device)
        adv3 = torch.Tensor(rollouts[idx_list[1]]['actions']).to(dtype).to(device)
        adv4 = torch.Tensor(rollouts[idx_list[2]]['actions']).to(dtype).to(device)

        return_diff2 = return_diffs[idx_list[0]]
        return_diff3 = return_diffs[idx_list[1]]
        return_diff4 = return_diffs[idx_list[2]]
        ...
        for z in range(self.num_subgroups - 1):
            self.nu[z][0] -= self.nu_lr * (self.epsilon - return_diff)
            self.nu[z][0] = max(min(self.nu[z][0], self.nu_max), 0)
            self.nu[z][1] -= self.nu_lr * (self.epsilon + return_diff)
            self.nu[z][1] = max(min(self.nu[z][1], self.nu_max), 0)

            self.nu[z][2] -= self.nu_lr * (self.epsilon - return_diff2)
            self.nu[z][2] = max(min(self.nu[z][2], self.nu_max), 0)
            self.nu[z][3] -= self.nu_lr * (self.epsilon + return_diff2)
            self.nu[z][3] = max(min(self.nu[z][3], self.nu_max), 0)

            self.nu[z][4] -= self.nu_lr * (self.epsilon - return_diff3)
            self.nu[z][4] = max(min(self.nu[z][4], self.nu_max), 0)
            self.nu[z][5] -= self.nu_lr * (self.epsilon + return_diff3)
            self.nu[z][5] = max(min(self.nu[z][5], self.nu_max), 0)

            self.nu[z][6] -= self.nu_lr * (self.epsilon - return_diff4)
            self.nu[z][6] = max(min(self.nu[z][6], self.nu_max), 0)
            self.nu[z][7] -= self.nu_lr * (self.epsilon + return_diff4)
        ...
        for epoch in range(self.num_epochs):
            ...
            for _, (obs_b, act_b, vtarg_b, adv_b,adv_b2, adv_b3, adv_b4, cvtarg_b, cadv_b,
                ...
                adv2_coefficient = 0
                for z in range(self.num_subgroups - 1):
                    adv2_coefficient += (-self.nu[z][2] + self.nu[z][3])

                adv3_coefficient = 0
                for z in range(self.num_subgroups - 1):
                    adv3_coefficient += (-self.nu[z][4] + self.nu[z][5])

                adv4_coefficient = 0
                for z in range(self.num_subgroups - 1):
                    adv4_coefficient += (-self.nu[z][6] + self.nu[z][7])
                
                # self.pi_loss = (kl_new_old - (1 / self.lam) * ratio * (adv_b - self.nu[0] * cadv_b[:,0,:])) \
                #           * (kl_new_old.detach() <= self.eta).type(dtype)


                policy_update_term = adv_b * adv_coefficient + adv_b2 * adv2_coefficient  + adv_b3 * adv3_coefficient  + adv_b4 * adv4_coefficient 
    ...
    class HalfCheetahRewardWrapper(gym.Wrapper):
        ...
        def step(self, action):
            ...
            if self.task_id == 0:
                ...
                obs = np.append(obs, [4, 5, 6, 7])
                # reward = reward - 2 * info['x_velocity']
                reward = reward - 0.5*info['x_velocity']
            elif self.task_id == 2:
        ...
        def reset(self, **kwargs):
            ...
            # TO-DO: reset for two other tasks
            if self.task_id == 0:
                obs = np.append(obs, [0, 1, 2, 3])
            elif self.task_id == 1:
                obs = np.append(obs, [4, 5, 6, 7])
        ...
        for z0 in range(num_subgroups):
            ...
            for t in range(num_tasks):
                for _ in range(args.rounds_of_update):
                    ...
                    other_group_return = fcpo[z0][t].update_params(rollouts[z0], dtype, device,return_diff,other_group_return, z0, t)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rounds-of-update',type=float, default=5,
