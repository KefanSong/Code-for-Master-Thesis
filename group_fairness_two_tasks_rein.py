...
class FOCOPS:
    ...
    def update_params(self, rollout, dtype, device,return_diff, other_group_return, group):
        ...
        wandb.log({"Group"+str(group)+"AvgR": np.mean(np.sort(self.score_queue))})
        
        # self.logger.update('nu0', self.nu[0])
        # self.logger.update('nu1', self.nu[1])

        print('logger AvgR', np.mean(self.score_queue))
        # Save models
    ...
    for iter in range(args.max_iter_num):
        ...
        for z0 in range(num_subgroups):
            ...
            for z1 in range(num_subgroups):
                ...
                other_group_return = rollouts[z1]['avg_return']

            
            
            other_group_return = fcpo[z0].update_params(rollouts[z0], dtype, device,return_diff,other_group_return, z0)
            agent = fcpo[z0].agent
            #step
