...
class DataGenerator:
    ...
    def run_traj(self, env, policy, value_net, cvalue_net, running_stat,
                 ...
                 dtype, device, constraint, task_id):
        ...
        while batch_idx < self.batch_size:
            ...
            for t in range(self.max_eps_len):
                ...
                if done or t == self.max_eps_len - 1:
                    ...
                    # TO-DO: log episodic return, with group_id in run_traj input
            ...
            # TO-DO: add value_net with task_id
            # Calculate advantage
            adv_eps, vtarg_eps = self.get_advantage(value_net, gamma, gae_lam, dtype, device, mode='reward', task_id=task_id)
            cadv_eps, cvtarg_eps = self.get_advantage(cvalue_net, c_gamma, c_gae_lam, dtype, device, mode='cost', task_id=task_id)
    ...
    # 
    def get_advantage(self, value_net, gamma, gae_lam, dtype, device, mode='reward', task_id=0):
        if mode == 'reward':
            ...
            for t in reversed(range(self.eps_len)):
                ...
                # print(self.task_id)
                current_val, next_val = torch_to_numpy(value_net(obs_tensor, task_id), value_net(next_obs_tensor, task_id))
            ...
            vtarg_eps = torch_to_numpy(value_net(obs_eps_tensor, task_id)) + adv_eps
